...
def get_security_incidents(days_back=30, include_title_filter=True, verbose=True):
    """
...
def main():
    ...
    parser.add_argument("--days", type=int, default=30, help="Number of days back to query (default: 30)")
    ...
    # Display total unique incidents by IncidentNumber (fallback to IncidentId)
    unique_incident_ids = {
        str(inc.get("IncidentNumber") or inc.get("IncidentId"))
        for inc in incidents
        if inc.get("IncidentNumber") or inc.get("IncidentId")
    }
    print(f"Total unique security incidents: {len(unique_incident_ids)}")
