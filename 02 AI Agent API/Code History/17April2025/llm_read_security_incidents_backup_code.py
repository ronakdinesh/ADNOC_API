...
class IncidentAnalysisOutput(BaseModel):
    ...
    def validate_and_transform_input(cls, data: Any) -> Dict[str, Any]:
        ...
        required_fields = {
            ...
            "metrics_panel": {},
            "threat_intel_context": "Not provided",
            "asset_impact_analysis": "Not provided",
            "historical_context": "Not provided"
        }
        ...
        # Fix technique_details if it's a list instead of a dictionary
        if 'technique_details' in cleaned_data and not isinstance(cleaned_data['technique_details'], dict):
            print(f"Converting technique_details from {type(cleaned_data['technique_details']).__name__} to dict")
            
            if isinstance(cleaned_data['technique_details'], list):
                # Convert list of dictionaries to the expected format
                fixed_techniques = {}
                for technique_item in cleaned_data['technique_details']:
                    if isinstance(technique_item, dict):
                        # Extract the technique ID from the name field if available
                        technique_id = None
                        if 'name' in technique_item and isinstance(technique_item['name'], str):
                            # Try to extract a technique ID like T1566 from a name like "T1566 (Phishing)"
                            id_match = re.search(r'(T\d+(?:\.\d+)?)', technique_item['name'])
                            if id_match:
                                technique_id = id_match.group(1)
                        
                        # If no ID found in name, check for an id field
                        if not technique_id and 'id' in technique_item:
                            technique_id = str(technique_item['id'])
                            # Ensure proper format (T followed by numbers)
                            if not technique_id.startswith('T'):
                                technique_id = f"T{technique_id}"
                        
                        # If still no ID, fall back to a generated ID
                        if not technique_id:
                            # Just generate a placeholder ID
                            technique_id = f"T{1000 + len(fixed_techniques)}"
                        
                        # Store under the proper ID
                        fixed_techniques[technique_id] = technique_item
                
                cleaned_data['technique_details'] = fixed_techniques
            else:
                # If it's neither a dict nor a list, create an empty dict
                cleaned_data['technique_details'] = {}
        
        # Ensure technique_details exists even if not provided
        if 'technique_details' not in cleaned_data:
            cleaned_data['technique_details'] = {}
...
def summarize_log_patterns_with_llm(patterns: Dict[str, Dict[str, Any]], domain: str = None, incident_info: Dict[str, Any] = None) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"Please analyze these log patterns and explain what they might indicate in simple, "
            f"human-readable terms. Focus on what these patterns might reveal about the security "
            f"incident and potential actions that might be needed.\n\n"
            f"{incident_context}\n"
            f"{pattern_text_str}\n\n"
            f"Provide your analysis of what these log patterns might indicate. Include observations about:\n"
            f"1. The significance of the destination IPs and ports (e.g., common services, suspicious destinations)\n"
            f"2. What the user activity might suggest (normal business operations vs. suspicious)\n"
            f"3. Any network traffic patterns that stand out\n"
            f"4. Context on whether this activity appears concerning or routine\n\n"
            f"Format your response as a clear, concise explanation for a security analyst to understand the significance of these patterns."
        )
...
def analyze_incident_context(incident_data: pd.DataFrame, all_incidents: Dict[str, pd.DataFrame] = None, log_window_days: int = 7) -> str:
    ...
    prompt = (
        ...
        f"5. AT-A-GLANCE METRICS DASHBOARD: Create a concise metrics panel summarizing critical stats (e.g., total updates, triage time, resolution time, number of log hits, number of indicators). Provide it as a dictionary of key‑value pairs.\n\n"
        
        f"6. THREAT INTELLIGENCE CONTEXT: Summarize any known threat actor associations or campaigns linked to the indicators or activity observed.\n\n"
        
        f"7. ASSET IMPACT ANALYSIS: Identify affected business units, specific systems, and types of data that may be impacted.\n\n"
        
        f"8. HISTORICAL CONTEXT: Review historical data (including the related incidents list provided) and note any previous incidents that share indicators or MITRE techniques with this incident.\n\n"
        ...
        f"- The response MUST include ALL of these fields: executive_summary, severity_indicator, correlation_matrix, attack_chain, risk_score, business_impact, metrics_panel, threat_intel_context, asset_impact_analysis, historical_context, threat_details, significance, recommended_actions, summary, attack_techniques, technique_details, severity_assessment, next_steps_for_l1, time_sensitivity, incident_type, potential_impact\n"
        ...
        f"Provide the analysis as a JSON object with keys for both the new components (executive_summary, severity_indicator, correlation_matrix, attack_chain, risk_score, business_impact, metrics_panel, threat_intel_context, asset_impact_analysis, historical_context) and the original fields (threat_details, significance, recommended_actions, summary, attack_techniques, technique_details, severity_assessment, next_steps_for_l1, time_sensitivity, incident_type, and potential_impact)."
    )
    ...
    try:
        ...
        try:
            ...
            # Fix technique_details issues
            if 'technique_details' in analysis_result and not isinstance(analysis_result['technique_details'], dict):
                print(f"Fixing technique_details format: {type(analysis_result['technique_details']).__name__} to dict")
                
                # If it's a list, try to convert to a dictionary
                if isinstance(analysis_result['technique_details'], list):
                    fixed_techniques = {}
                    for technique_item in analysis_result['technique_details']:
                        if isinstance(technique_item, dict):
                            # Extract the technique ID from the name field if available
                            technique_id = None
                            if 'name' in technique_item and isinstance(technique_item['name'], str):
                            ...
                            # If still no ID, use a fallback ID
                            if not technique_id:
                                # Just generate a unique placeholder ID
                                technique_id = f"T{1000 + len(fixed_techniques)}"
                            
                            # Store under the proper ID
                            fixed_techniques[technique_id] = technique_item
                    
                    analysis_result['technique_details'] = fixed_techniques
                else:
                    # If it's something other than a list or dict, use an empty dict
                    analysis_result['technique_details'] = {}
                    
            # Ensure technique_details exists even if not provided
            if 'technique_details' not in analysis_result:
                analysis_result['technique_details'] = {}
        ...
        analysis_content = (
            ...
            f"EXECUTIVE DASHBOARD (At‑a‑Glance Metrics):\n"
            f"==========================================\n"
            + "\n".join([f"* {k}: {v}" for k, v in validated_result.metrics_panel.items()]) + "\n\n"
            
            f"THREAT INTELLIGENCE CONTEXT:\n"
            f"============================\n"
            f"{validated_result.threat_intel_context}\n\n"
            
            f"ASSET IMPACT ANALYSIS:\n"
            f"=======================\n"
            f"{validated_result.asset_impact_analysis}\n\n"
            
            f"HISTORICAL CONTEXT:\n"
            f"===================\n"
            f"{validated_result.historical_context}\n\n"
    ...
    # Create incident context for LLM interpretation
    incident_info = {
        "severity": last_row['Severity'],
        "status": last_row['Status'],
        "incident_type": incident_type_formatted if 'incident_type_formatted' in locals() else "Unknown", 
        "title": title_text,
        "incident_number": incident_number
    }
...
def analyze_security_incidents(excel_path: str, tenant_id: str = None, fetch_time: datetime = None, log_window_days: int = 7) -> None:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Analyze security incidents from Excel file')
