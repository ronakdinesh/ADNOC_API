...
class IncidentAnalysisOutput(BaseModel):
    ...
    attack_techniques: Optional[List[str]] = Field(default=[], description="Identified potential MITRE ATT&CK techniques")
    severity_assessment: Optional[str] = Field(default=None, description="LLM's assessment of the incident severity")
    next_steps_for_l1: Optional[List[str]] = Field(default=[], description="Specific next steps for L1 SOC analyst")
    time_sensitivity: Optional[str] = Field(default=None, description="How quickly the incident should be addressed")
    incident_type: Optional[str] = Field(default=None, description="Classification of the incident type (e.g., malware, phishing)")
...
def get_mitre_attack_info(technique_ids: List[str]) -> str:
    """Provide information about MITRE ATT&CK techniques"""
    # Common MITRE ATT&CK techniques and their descriptions
    mitre_techniques = {
        "T1566": {
            "name": "Phishing",
        ...
        "T1566.001": {
            ...
            "mitigation": "Anti-virus, user training, attachment sandboxing, disable macros"
        },
        "T1566.002": {
            "name": "Spearphishing Link",
            "tactic": "Initial Access",
            "description": "Adversaries may send spearphishing emails with a malicious link to gain access.",
            "mitigation": "URL filtering, user training, browser isolation"
        },
        "T1078": {
            "name": "Valid Accounts",
        ...
        "T1486": {
            ...
            "mitigation": "Backups, application controls, behavior prevention"
        },
        "T1059": {
            "name": "Command and Scripting Interpreter",
            "tactic": "Execution",
            "description": "Adversaries may use command-line interfaces or scripting languages for execution.",
            "mitigation": "Execution prevention, script blocking, behavior monitoring"
        },
        "T1053": {
            "name": "Scheduled Task/Job",
            "tactic": "Execution, Persistence, Privilege Escalation",
            "description": "Adversaries may use task scheduling functionality for execution.",
            "mitigation": "Privileged account management, execution prevention"
        },
        "T1083": {
            "name": "File and Directory Discovery",
            "tactic": "Discovery",
            "description": "Adversaries may enumerate files and directories to find important information.",
            "mitigation": "File permissions, limit directory listings, process monitoring"
        },
        "T1087": {
            "name": "Account Discovery",
            "tactic": "Discovery",
            "description": "Adversaries may attempt to get a listing of user accounts on a system.",
            "mitigation": "Operating system configuration, account monitoring"
        },
        "T1046": {
            "name": "Network Service Discovery",
            "tactic": "Discovery",
            "description": "Adversaries may attempt to identify services running on hosts.",
            "mitigation": "Network intrusion prevention, network segmentation"
        },
        "T1133": {
            "name": "External Remote Services",
            "tactic": "Initial Access, Persistence",
            "description": "Adversaries may use external remote services as entry points.",
            "mitigation": "MFA, network segmentation, VPN restrictions"
        },
        "T1078.004": {
            "name": "Cloud Accounts",
            "tactic": "Defense Evasion, Persistence, Privilege Escalation, Initial Access",
            "description": "Adversaries may use cloud accounts for initial access or persistence.",
            "mitigation": "MFA, least privilege, account monitoring for cloud services"
        }
    }
    
    # Unknown technique info
    unknown_technique = {
        "name": "Unknown Technique",
        "tactic": "Unknown",
        "description": "This technique ID was not found in the local database.",
        "mitigation": "Refer to the MITRE ATT&CK website for more information."
    }
    ...
    for technique_id in technique_ids:
        ...
        # Get technique info or default to unknown
        technique_info = mitre_techniques.get(technique_id, unknown_technique)
        
        # Add to output
        formatted_info += (
            f"[{technique_id}] {technique_info['name']} - {technique_info['tactic']}\n"
            f"  Description: {technique_info['description']}\n"
            f"  Mitigation: {technique_info['mitigation']}\n\n"
        )
...
def analyze_incident_context(incident_data: pd.DataFrame, all_incidents: Dict[str, pd.DataFrame] = None) -> str:
    ...
    prompt = (
        ...
        f"When identifying MITRE ATT&CK techniques, please provide technique IDs like T1566 (Phishing) where applicable.\n\n"
        f"Provide the analysis as a JSON object with keys: 'threat_details', 'significance', 'recommended_actions' (as a list of strings), 'summary', 'attack_techniques' (as a list of strings like 'T1566', 'T1078'), 'severity_assessment', 'next_steps_for_l1' (as a list of strings), 'time_sensitivity' (how urgent this incident is), and 'incident_type' (classification of incident - e.g. malware, phishing, unauthorized access)."
    )
    ...
    try:
        ...
        if validated_result.attack_techniques:
            mitre_info = get_mitre_attack_info(validated_result.attack_techniques)
        ...
        # Quick assessment section with time sensitivity
        quick_assessment = (
            f"QUICK ASSESSMENT:\n"
            f"Incident Type: {validated_result.incident_type or 'Unclassified'}\n"
            f"Severity Assessment: {validated_result.severity_assessment or 'Not provided'}\n"
            f"Time Sensitivity: {validated_result.time_sensitivity or 'Not specified'}\n"
