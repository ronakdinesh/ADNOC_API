...
def analyze_incident_context(incident_data: pd.DataFrame, all_incidents: Dict[str, pd.DataFrame] = None, log_window_days: int = 7) -> str:
    ...
    try:
        ...
        try:
            ...
            print(f"Error generating analysis with LLM for incident {incident_number}: {str(e)}")
            print("Attempting to correct data format issues...")
            
            # Fix the specific issues observed in the error message
            for field in ['threat_intel_context', 'asset_impact_analysis', 'historical_context', 'threat_details']:
                if field in analysis_result and isinstance(analysis_result[field], list):
                    if len(analysis_result[field]) > 0:
                        if all(isinstance(item, dict) for item in analysis_result[field]):
                            # Convert the list of dictionaries to a single dictionary
                            print(f"Converting {field} from list of dicts to a single dict")
                            analysis_result[field] = {k: v for item in analysis_result[field] for k, v in item.items()}
                        else:
                            # Convert to string
                            print(f"Converting {field} from list to string")
                            analysis_result[field] = str(analysis_result[field])
            
            # Convert any other problematic list fields to appropriate types
            for field in ['risk_score', 'business_impact']:
                if field in analysis_result and isinstance(analysis_result[field], (int, float)):
                    print(f"Converting {field} from {type(analysis_result[field]).__name__} to dict")
                    analysis_result[field] = {"value": analysis_result[field]}
                elif field in analysis_result and isinstance(analysis_result[field], list):
                    print(f"Converting {field} from list to dict")
                    if all(isinstance(item, dict) for item in analysis_result[field]):
                        analysis_result[field] = {k: v for item in analysis_result[field] for k, v in item.items()}
                    else:
                        analysis_result[field] = {"items": analysis_result[field]}
            ...
            try:
                ...
                print(f"Still facing validation issues after fixes: {str(e2)}")
                # Create a minimal valid object for fallback
                validated_result = IncidentAnalysisOutput(
                    executive_summary="Could not generate a valid analysis due to formatting issues.",
                    severity_indicator="Medium",
                    threat_details="Analysis validation failed. Please check logs for details.",
                    next_steps_for_l1=["Review the incident manually", "Consult senior analyst"]
                )
