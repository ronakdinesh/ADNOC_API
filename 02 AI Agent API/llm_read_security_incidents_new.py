...
def format_soc_analyst_report(analysis_output: IncidentAnalysisOutput) -> str:
    ...
    # Add executive summary and severity indicator if available
    if analysis_output.executive_summary:
        report.append(f"Executive Summary: {analysis_output.executive_summary}")
    
    if analysis_output.severity_indicator:
        report.append(f"Severity: {analysis_output.severity_indicator}")
        
    report.append("\n")
    
    # Format immediate actions section with the exact headers requested
    ...
    if analysis_output.immediate_actions:
        for action in analysis_output.immediate_actions:
            ...
            action_status = action.get("status", "Recommended")
            report.append(f"- {action_desc} [{action_status}]")
    else:
        # If no immediate actions specified, use recommended_actions with default action status
        if analysis_output.recommended_actions:
            for action in analysis_output.recommended_actions:
                report.append(f"- {action} [Recommended]")
        else:
            report.append("- No immediate actions provided")
    
    report.append("\n")
    
    # Format future steps section with the exact headers requested
    ...
    if analysis_output.future_steps:
        for step in analysis_output.future_steps:
            ...
            # Add data points as sub-bullets if available
            if data_points:
                for point in data_points:
                    report.append(f"  * {point}")
    else:
        # If no future steps specified, use next_steps_for_l1 as a fallback
        if analysis_output.next_steps_for_l1:
            for step in analysis_output.next_steps_for_l1:
                report.append(f"- {step}")
        else:
            report.append("- No future steps provided")
    
    # Join all parts of the report with newlines
    return "\n".join(report)

def generate_soc_analyst_prompt(incident_data: Dict[str, Any], logs: List[Dict[str, Any]], 
                               indicators: SecurityIndicators, alerts: List[Dict[str, Any]]) -> str:
    """
...
def generate_soc_analyst_report(incident_data: Dict[str, Any], logs: List[Dict[str, Any]], 
                              indicators: SecurityIndicators, alerts: List[Dict[str, Any]]) -> str:
    """
    Generate a SOC analyst report with immediate actions and future steps.
    ...
    try:
        ...
        prompt = generate_soc_analyst_prompt(incident_data, logs, indicators, alerts)
        
        # Configure ollama client with the right base URL
        client = ollama.Client(host=OLLAMA_API_BASE)
        
        print("Generating SOC analyst report...")
        
        # Make the API call to Ollama with format=json to try to get structured output
        response = client.chat(
            model=OLLAMA_MODEL,
            messages=[{"role": "user", "content": prompt}],
            stream=False,
        ...
        try:
            ...
            # Create IncidentAnalysisOutput object from the response
            analysis_output = IncidentAnalysisOutput(**analysis_data)
            
            # Format the output using our format_soc_analyst_report function
            formatted_report = format_soc_analyst_report(analysis_output)
            return formatted_report
            
        except json.JSONDecodeError:
            # If LLM didn't return valid JSON, try to extract structured data manually
            print("LLM didn't return valid JSON. Attempting to extract structured data...")
            
            # Create a minimal IncidentAnalysisOutput with extracted data
            analysis_output = IncidentAnalysisOutput(
                executive_summary="Error parsing LLM response as JSON. Please check the raw output.",
                severity_indicator="Unknown",
                recommended_actions=["Review the raw LLM output and extract actions manually"],
                next_steps_for_l1=["Review the raw LLM output and extract future steps manually"]
            )
            
            # Append the raw output to the formatted report
            formatted_report = format_soc_analyst_report(analysis_output)
            formatted_report += "\n\nRAW LLM RESPONSE:\n" + response_content
        ...
        print(f"Error generating SOC analyst report: {str(e)}")
        return f"Error generating SOC analyst report: {str(e)}"
